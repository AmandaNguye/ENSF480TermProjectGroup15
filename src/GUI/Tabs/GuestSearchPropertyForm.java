...
public class GuestSearchPropertyForm extends GUI {
	...
	public GuestSearchPropertyForm() {
		...
		String[] types = { "Any", "Apartment", "Attached House", "Detached House", "Townhouse", };
		...
		searchButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int bedrooms = Integer.parseInt(bedroomsBox.getSelectedItem().toString());
				int bathrooms = Integer.parseInt(bathroomsBox.getSelectedItem().toString());
				String quadrant = quadrantBox.getSelectedItem().toString();
				Boolean furnished = furnishedBox.isSelected();
				List<Property> properties = null;
				try {
					properties = rentalDAO.searchProperties(type, bedrooms, bathrooms, quadrant, furnished);
					SearchPropertyTableModel model = new SearchPropertyTableModel(properties);
					propertyTable.setModel(model);
				} catch (Exception exc) {
					JOptionPane.showMessageDialog(GuestSearchPropertyForm.this, "Error: " + exc, "Error",

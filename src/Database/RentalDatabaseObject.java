...
public class RentalDatabaseObject {
	...
	public void enterUser(User newUser) throws Exception {
		...
		try {
			query = myConn.prepareStatement("INSERT INTO users (username, password, accounttype) VALUES (?, ?, ?)");
	...
	public Property getProperty(String address) throws Exception {
		...
		try {
			query = myConn.prepareStatement("SELECT * FROM properties WHERE address = ?");
	...
	public String getPropertyLandlord(String address) throws Exception {
		...
		try {
			query = myConn.prepareStatement("SELECT owner FROM properties WHERE address = ?");
			query.setString(1, address);
	 ...
	 */
	public void enterEmail(String sender, String receiver, String message) throws Exception {
		PreparedStatement query = null;
		ResultSet results = null;

		try {
			query = myConn.prepareStatement("INSERT INTO emails (sender, receiver, message) VALUES (?, ?, ?)");
	 ...
	 * @param 	message		email data
	 */
	public void deleteEmail(String sender, String receiver, String message) throws Exception {
		PreparedStatement query = null;
		ResultSet results = null;

		try {
			query = myConn.prepareStatement("DELETE FROM emails WHERE sender = ? AND receiver = ? AND message = ?");
			query.setString(1, sender);
			query.setString(2, receiver);
			query.setString(3, message);
			System.out.println(query.toString());
	 ...
	 * @return 	property data
	 */
	public List<Property> getLandlordProperties(String landlord) throws Exception {
		updateAllPropertyStatus();
	 ...
	 * @return 	property data
	 */
	public List<Property> searchProperties(String type, int bedrooms, int bathrooms, String quadrant, Boolean furnished)
			throws Exception {
		updateAllPropertyStatus();
		List<Property> list = new ArrayList<>();
	 ...
	 */
	public void changePropertyExpiry(String address, String orignalExpiry, int extendedDays) throws Exception {
		PreparedStatement query = null;
		ResultSet results = null;

		try {
			query = myConn.prepareStatement("UPDATE properties SET expirydate = ? WHERE address = ?");
	 ...
	 */
	public void changePropertyStatus(String address, String status) throws Exception {
		PreparedStatement query = null;
		ResultSet results = null;
		...
		try {
			query = myConn.prepareStatement("UPDATE properties SET status = ? WHERE address = ?");
	...
	private void updateAllPropertyStatus() throws Exception {
		...
		ResultSet results = null;
		try {
			query = myConn.prepareStatement("SELECT * FROM properties WHERE status = 'active' AND expirydate <= ?");
		...
		try {
			query = myConn
					.prepareStatement("SELECT * FROM properties WHERE status = 'payment required' AND expirydate > ?");
			query.setString(1, java.time.LocalDate.now().toString());
			System.out.println(query.toString());
			results = query.executeQuery();
			while (results.next()) {
	...
	private Property convertRowToProperty(ResultSet set) throws SQLException {
		...
		Property tempProperty = new Property(id, address, owner, type, bedrooms, bathrooms, furnishedBool, quadrant,
				status, expirydate);
	 ...
	 * @param 	furnished		if the property should be furnished
	 */
	public void addSubscription(String renter, String type, int bedrooms, int bathrooms, String quadrant,
			Boolean furnished) throws Exception {
		PreparedStatement query = null;
		ResultSet results = null;
	...
	public void setFeeInfo(int newPrice, int newPeriod) throws Exception {
		...
		try {
			query = myConn.prepareStatement(
					"REPLACE INTO feeinfo (price, perioddays) VALUES (" + newPrice + "," + newPeriod + ")");
			System.out.println(query.toString());
			int rowcount = query.executeUpdate();
			System.out.println("Success - " + rowcount + " rows affected.");
		} finally {
	...
	public void notifySubscription(Property property) throws Exception {
		...
		try {
			...
			tempquery += " AND (quadrant = 'any' OR quadrant = '" + property.getQuadrant() + "')";
			int furnishedint = 0;
			...
			while (results.next()) {
				Subscription tempSub = convertRowToSubscription(results);
				enterNotification(tempSub.getId(), property.getId(), tempSub.getRenter());
			}
		} finally {
			close(query, results);
		}
	...
	private void pruneNotifications(Property property) throws SQLException {
		...
		try {
			query = myConn.prepareStatement("DELETE FROM notifications WHERE listingid = " + property.getId());
			System.out.println(query.toString());
			int rowcount = query.executeUpdate();
			System.out.println("Success - " + rowcount + " rows affected.");
		} finally {
	...
	private void pruneNotifications(Subscription subscription) throws SQLException {
		...
		try {
			query = myConn.prepareStatement("DELETE FROM notifications WHERE subscriptionid = " + subscription.getId());
			System.out.println(query.toString());
			int rowcount = query.executeUpdate();
			System.out.println("Success - " + rowcount + " rows affected.");
		} finally {
	...
	private Subscription convertRowToSubscription(ResultSet set) throws SQLException {
		...
		Subscription tempSub = new Subscription(id, renter, type, bedrooms, bathrooms, furnishedBool, quadrant);
	...
	public void deleteSubscription(int id) throws Exception {
		...
		try {
			query = myConn.prepareStatement("DELETE FROM subscriptions WHERE id = " + id);
			System.out.println(query.toString());
			int rowcount = query.executeUpdate();
			System.out.println("Success - " + rowcount + " rows affected.");
		} finally {
	 ...
	 */
	public List<Notification> getRentersNotifications(String renter) throws Exception {
		...
		try {
			query = myConn.prepareStatement("SELECT * FROM notifications WHERE renter = ?");
			query.setString(1, renter);
	 ...
	 */
	private Notification convertRowToNotification(ResultSet set) throws SQLException {
		int subscriptionid = set.getInt("subscriptionid");
		int listingid = set.getInt("listingid");
		String renter = set.getString("renter");

		Notification tempNotification = new Notification(subscriptionid, listingid, renter);
	 ...
	 * @param 	renter				renter's username
	 */
	public void deleteNotification(int subscriptionid, String address, String renter) throws Exception {
		PreparedStatement query = null;
		ResultSet results = null;
	 ...
	 * @param 	renter				renter's username
	 */
	public void enterNotification(int subscriptionid, int listingid, String renter) throws Exception {
		PreparedStatement query = null;
		ResultSet results = null;

		try {
			query = myConn
					.prepareStatement("INSERT INTO notifications (subscriptionid, listingid, renter) VALUES (?, ?, ?)");
			query.setInt(1, subscriptionid);
	...
	public void initializeLogs() throws Exception {
		...
		for (Property p : list) {
			if (getNumberOfPropertyLogs(p) == 0)
				addLog(p);
		}
	}
	...
	private void addLog(Property property) throws Exception {
		...
		try {
			query = myConn
					.prepareStatement("INSERT INTO propertylogs (listingid, statuschange, logdate) VALUES (?, ?, ?)");
	...
	private int getNumberOfPropertyLogs(Property property) throws Exception {
		...
		try {
			query = myConn.prepareStatement("SELECT * FROM propertylogs WHERE listingid = ?");
			query.setInt(1, property.getId());
	 ...
	 */
	public List<Property> getPeriodLogs(String statuschange, int perioddays) throws Exception {
		List<Property> list = new ArrayList<>();
		...
		try {
			query = myConn.prepareStatement("SELECT * FROM propertylogs WHERE statuschange = ? AND logdate >= ?");
			query.setString(1, statuschange);
			query.setString(2, java.time.LocalDate.now().minusDays(perioddays).toString());
	...
	public List<Property> getProperties(String status) throws Exception {
		...
		try {
			query = myConn.prepareStatement("SELECT * FROM properties WHERE status = ?");
			query.setString(1, status);
